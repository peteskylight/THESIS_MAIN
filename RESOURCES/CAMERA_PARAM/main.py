...
cameraInput = RTSP_URL #JUST CHANGE IF CAMERA OR RSTP
...
def main():
    ...
    #camera = cv2.VideoCapture(cameraInput)  # Use the specified camera IN NUMBER
    
    camera = = cv2.VideoCapture(RTSP_URL, cv2.CAP_FFMPEG) #FOR RSTP CAMERA
    camera.set(cv2.CAP_PROP_FRAME_WIDTH, frameWidth)
    camera.set(cv2.CAP_PROP_FRAME_HEIGHT, frameHeight)
    
    # Initialize YOLO-POSE model
    model = YOLO("yolov8n-pose.pt", device = 'gpu') #---- GPU IMPORTANT!!!
    model2 = YOLO("yolov8n.pt", device = 'gpu') #--------GPU IMPORTANT!!!!
